 ...
 * objects, name/value pairs are represented by a single token. <h3>Parsing JSON</h3> To create a
 * recursive descent parser for your own JSON streams, first
 ...
 * </pre>
 * 
 * <h3>Number Handling</h3> This reader permits numeric values to be read as strings and string
 * values to be read as numbers. For example, both elements of the JSON array {@code [1, "1"]} may
 * be read using either {@link #nextInt} or {@link #nextString}. This behavior is intended to
 * prevent lossy numeric conversions: double is JavaScript's only numeric type and very large values
 * like {@code 9007199254740993} cannot be represented exactly on that platform. To minimize
...
public final class JsonReader implements Closeable {
  ...
  public JsonToken peek() throws IOException {
    ...
    switch (peekStack()) {
      case EMPTY_DOCUMENT:
        ...
        if (!lenient && token != JsonToken.BEGIN_ARRAY
   ...
   *          and valueLength fields and return a null result. This only
   *          works if the literal is short; a string is returned otherwise.
   */
  private String nextLiteral(boolean assignOffsetsOnly) throws IOException {
    ...
    findNonLiteralCharacter: while (true) {
      ...
      /*
       * Attempt to load the entire literal into the buffer at once. If we
       * run out of input, add a non-literal character at the end so that
       * decoding doesn't need to do bounds checks.
       */
      if (i < buffer.length) {
        if (fillBuffer(i + 1)) {
          continue;
      ...
      // use a StringBuilder when the value is too long. It must be an
      // unquoted string.
      if (builder == null) {
        builder = new StringBuilder();
      }
   ...
   * in the fraction.
   */
  private static JsonToken decodeNumber(char[] chars, int offset, int length) {
    int i = offset;
    int c = chars[i];
